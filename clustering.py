    ...
    if uploaded_file is not None:
        try:
            ...
            if len(selected_features) not in [2, 3]:
                ...
                # Step 2.1: Check for NaN or infinite values in selected columns
                if df[selected_features].isnull().values.any():
                    st.warning("The selected columns contain missing values (NaN). These will be dropped.")
                    df = df.dropna(subset=selected_features)
                
                if np.any(np.isinf(df[selected_features].values)):
                    st.warning("The selected columns contain infinite values. These will be replaced with NaN.")
                    df[selected_features] = df[selected_features].replace([np.inf, -np.inf], np.nan)
                    df = df.dropna(subset=selected_features)

                # Proceed with clustering if the data is cleaned
