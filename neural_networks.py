...
def run():
    ...
    if uploaded_file is not None:
        ...
        if uploaded_test_file is not None:
            ...
            # Ensure y_val_pred is 1D for binary classification, or use argmax for multi-class
            if num_classes == 2:
                y_val_pred = (y_val_pred > 0.5).astype(int).flatten()  # Ensure it's a 1D array
            else:
                y_val_pred = np.argmax(y_val_pred, axis=1)  # For multi-class, use argmax to get the class index

            # Now, make sure that both y_val and y_val_pred are of integer type for confusion matrix
            y_val = y_val.astype(int)

            # Initialize the LabelEncoder for multi-class labels if needed
            if len(np.unique(y)) > 2:
                le = LabelEncoder()
                le.fit(y)  # Fit on the original labels (y)

            cm = confusion_matrix(y_val, y_val_pred)

            st.subheader("Confusion Matrix")
            fig_cm = plt.figure(figsize=(8, 6))  # Increase the size for better visibility
            sns.heatmap(cm, annot=True, cmap="Blues", 
                        xticklabels=le.classes_ if len(np.unique(y)) > 2 else ["0", "1"], 
                        yticklabels=le.classes_ if len(np.unique(y)) > 2 else ["0", "1"],
